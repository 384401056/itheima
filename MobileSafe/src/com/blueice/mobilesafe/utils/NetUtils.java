...
public class NetUtils {
	...
	private static void readAPN(Context context) {
		...
		//获取APN列表的Uri.
		Uri APN_URL = Uri.parse("context://telephony/carriers/preferapn"); //谷歌的4.0手机对该操作的权限进行了屏蔽。模拟器也是。
	...
	private static boolean isMobileConnection(Context context) {
		...
		// 获取Mobile的信息类。
		NetworkInfo networkType = manager.getNetworkInfo(ConnectivityManager.TYPE_MOBILE);
	...
	private static boolean isWifiConnection(Context context) {
		...
		// 获取wifi的信息类。
		NetworkInfo networkType = manager.getNetworkInfo(ConnectivityManager.TYPE_WIFI);
